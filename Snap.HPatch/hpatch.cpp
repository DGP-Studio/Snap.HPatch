...
#define hpatch_TRUE ((hpatch_BOOL)(!hpatch_FALSE))
    ...
    // 0: no error; other: saved errno value;
    typedef int hpatch_FileError_t;
...
#define _hpatch_align_upper(p, align2pow) _hpatch_align_lower(((hpatch_size_t)(p)) + ((align2pow) - 1), align2pow)
    ...
    typedef struct hpatch_TStreamInput
    {
        ...
        // stream size,max readable range;
    ...
    typedef struct hpatch_TStreamOutput
    {
        ...
        // stream size,max writable range; not is write pos!
        hpatch_StreamPos_t streamSize;
...
#define hpatch_kMaxPluginTypeLength 259
    ...
    typedef struct hpatch_compressedDiffInfo
    {
        ...
        // need open hpatch_decompressHandle number
        hpatch_uint compressedCount;
    ...
    typedef struct hpatch_TDecompress
    {
        ...
        hpatch_decompressHandle(*open)(struct hpatch_TDecompress* decompressPlugin, hpatch_StreamPos_t dataSize, const struct hpatch_TStreamInput* codeStream, hpatch_StreamPos_t code_begin, hpatch_StreamPos_t code_end);
        hpatch_BOOL(*close)(struct hpatch_TDecompress* decompressPlugin, hpatch_decompressHandle decompressHandle);

        // decompress_part() must out (out_part_data_end-out_part_data), otherwise error return hpatch_FALSE
        hpatch_BOOL(*decompress_part)(hpatch_decompressHandle decompressHandle, unsigned char* out_part_data, unsigned char* out_part_data_end);

        // reset_code add new compressed data; for support vcpatch, can NULL
        hpatch_BOOL(*reset_code)(hpatch_decompressHandle decompressHandle, hpatch_StreamPos_t dataSize, const struct hpatch_TStreamInput* codeStream, hpatch_StreamPos_t code_begin, hpatch_StreamPos_t code_end);

        // if you used decError value, once patch must used it's own hpatch_TDecompress
        volatile hpatch_dec_error_t decError;
    } hpatch_TDecompress;

#define _hpatch_update_decError(decompressPlugin,errorCode) \
    do \
    { \
        if ((decompressPlugin)->decError == hpatch_dec_ok) \
        { \
            (decompressPlugin)->decError = errorCode; \
        } \
    } \
    while((void)0, 0)

    const hpatch_TStreamInput* mem_as_hStreamInput(hpatch_TStreamInput* out_stream, const unsigned char* mem, const unsigned char* mem_end);
    const hpatch_TStreamOutput* mem_as_hStreamOutput(hpatch_TStreamOutput* out_stream, unsigned char* mem, unsigned char* mem_end);

    hpatch_BOOL hpatch_deccompress_mem(hpatch_TDecompress* decompressPlugin, const unsigned char* code, const unsigned char* code_end, unsigned char* out_data, unsigned char* out_data_end);
    ...
    void TStreamInputClip_init(TStreamInputClip* self, const hpatch_TStreamInput* srcStream, hpatch_StreamPos_t clipBeginPos, hpatch_StreamPos_t clipEndPos);
    ...
    // clip srcStream from clipBeginPos to clipEndPos as a new StreamInput;
    void TStreamOutputClip_init(TStreamOutputClip* self, const hpatch_TStreamOutput* srcStream, hpatch_StreamPos_t clipBeginPos, hpatch_StreamPos_t clipEndPos);

#define hpatch_kMaxPackedUIntBytes ((sizeof(hpatch_StreamPos_t) * 8 + 6) / 7 + 1)

    hpatch_BOOL hpatch_packUIntWithTag(unsigned char** out_code, unsigned char* out_code_end, hpatch_StreamPos_t uValue, hpatch_uint highTag, const hpatch_uint kTagBit);
    hpatch_uint hpatch_packUIntWithTag_size(hpatch_StreamPos_t uValue, const hpatch_uint kTagBit);

#define hpatch_packUInt(out_code, out_code_end, uValue) hpatch_packUIntWithTag(out_code, out_code_end, uValue, 0, 0)
#define hpatch_packUInt_size(uValue) hpatch_packUIntWithTag_size(uValue, 0)

    hpatch_BOOL hpatch_unpackUIntWithTag(const unsigned char** src_code, const unsigned char* src_code_end, hpatch_StreamPos_t* result, const hpatch_uint kTagBit);

#define hpatch_unpackUInt(src_code, src_code_end, result) hpatch_unpackUIntWithTag(src_code,src_code_end,result,0)
    ...
    typedef struct hpatch_TOutputCovers
    {
        ...
        // for support search covers by multi-thread
        void (*collate_covers)(struct hpatch_TOutputCovers* out_covers);
    ...
    static hpatch_inline void _singleDiffInfoToHDiffInfo(hpatch_compressedDiffInfo* out_diffInfo, const hpatch_singleCompressedDiffInfo* singleDiffInfo)
    {
    ...
    typedef struct sspatch_listener_t
    {
        ...
        // out_decompressPlugin: find decompressPlugin by info->compressType
        // out_temp_cache: *out_temp_cacheEnd-*out_temp_cache == info->stepMemSize + (I/O cache memory)
        // note: (I/O cache memory) >= hpatch_kStreamCacheSize*3
        hpatch_BOOL(*onDiffInfo)(struct sspatch_listener_t* listener, const hpatch_singleCompressedDiffInfo* info, hpatch_TDecompress** out_decompressPlugin, unsigned char** out_temp_cache, unsigned char** out_temp_cacheEnd);

        // onPatchFinish can null
        void (*onPatchFinish)(struct sspatch_listener_t* listener, unsigned char* temp_cache, unsigned char* temp_cacheEnd);
    } sspatch_listener_t;
    ...
    typedef struct sspatch_coversListener_t
    {
        ...
        // can NULL, data(in covers_cache) will invalid
        void (*onStepCoversReset)(struct sspatch_coversListener_t* listener, hpatch_StreamPos_t leaveCoverCount);
        void (*onStepCovers)(struct sspatch_coversListener_t* listener, const unsigned char* covers_cache, const unsigned char* covers_cacheEnd);
    ...
    static hpatch_inline void sspatch_covers_init(sspatch_covers_t* self)
    {
        memset(self, 0, sizeof(*self));
    }

    static hpatch_inline void sspatch_covers_setCoversCache(sspatch_covers_t* self, const unsigned char* covers_cache, const unsigned char* covers_cacheEnd)
    {
        self->covers_cache = covers_cache;
        self->covers_cacheEnd = covers_cacheEnd;
    }

    static hpatch_inline hpatch_BOOL sspatch_covers_isHaveNextCover(const sspatch_covers_t* self)
    {
        return (self->covers_cache != (self)->covers_cacheEnd);
    }
    ...
    hpatch_BOOL patch(unsigned char* out_newData, unsigned char* out_newData_end, const unsigned char* oldData, const unsigned char* oldData_end, const unsigned char* serializedDiff, const unsigned char* serializedDiff_end);
    ...
    // serializedDiff create by create_diff()
    // out_newData: sequential write
    // oldData: random read
    // serializedDiff: random read
    hpatch_BOOL patch_stream(const hpatch_TStreamOutput* out_newData, const hpatch_TStreamInput* oldData, const hpatch_TStreamInput* serializedDiff);
    ...
    // oldData: random read
    // serializedDiff: random read
    hpatch_BOOL patch_stream_with_cache(const hpatch_TStreamOutput* out_newData, const hpatch_TStreamInput* oldData, const hpatch_TStreamInput* serializedDiff, unsigned char* temp_cache, unsigned char* temp_cache_end);

    // get compressedDiff info
    // compressedDiff created by create_compressed_diff() or create_compressed_diff_stream()
    hpatch_BOOL getCompressedDiffInfo(hpatch_compressedDiffInfo* out_diffInfo, const hpatch_TStreamInput* compressedDiff);

    // see getCompressedDiffInfo()
    static hpatch_inline hpatch_BOOL getCompressedDiffInfo_mem(hpatch_compressedDiffInfo* out_diffInfo, const unsigned char* compressedDiff, const unsigned char* compressedDiff_end)
    {
    ...
    // out_newData: sequential write
    // oldData: random read
    // compressedDiff: random read
    hpatch_BOOL patch_decompress(const hpatch_TStreamOutput* out_newData, const hpatch_TStreamInput* oldData, const hpatch_TStreamInput* compressedDiff, hpatch_TDecompress* decompressPlugin);
    ...
    // oldData: random read
    // compressedDiff: random read
    hpatch_BOOL patch_decompress_with_cache(const hpatch_TStreamOutput* out_newData, const hpatch_TStreamInput* oldData, const hpatch_TStreamInput* compressedDiff, hpatch_TDecompress* decompressPlugin, unsigned char* temp_cache, unsigned char* temp_cache_end);

    // see patch_decompress()
    static hpatch_inline hpatch_BOOL patch_decompress_mem(unsigned char* out_newData, unsigned char* out_newData_end, const unsigned char* oldData, const unsigned char* oldData_end, const unsigned char* compressedDiff, const unsigned char* compressedDiff_end, hpatch_TDecompress* decompressPlugin)
    {
    ...
    static hpatch_inline void hpatch_coverList_init(hpatch_TCoverList* coverList)
    {
        assert(coverList != 0);
    ...
    // serializedDiff create by create_diff()
    hpatch_BOOL hpatch_coverList_open_serializedDiff(hpatch_TCoverList* out_coverList, const hpatch_TStreamInput* serializedDiff);

    // compressedDiff create by create_compressed_diff() or create_compressed_diff_stream()
    hpatch_BOOL hpatch_coverList_open_compressedDiff(hpatch_TCoverList* out_coverList, const hpatch_TStreamInput* compressedDiff, hpatch_TDecompress* decompressPlugin);

    static hpatch_inline hpatch_BOOL hpatch_coverList_close(hpatch_TCoverList* coverList)
    {
    ...
    // listener: call back when got diffInfo
    // out_newData: sequential write
    // oldData: random read
    ...
    hpatch_BOOL patch_single_stream(sspatch_listener_t* listener, const hpatch_TStreamOutput* out_newData, const hpatch_TStreamInput* oldData, const hpatch_TStreamInput* singleCompressedDiff, hpatch_StreamPos_t  diffInfo_pos, sspatch_coversListener_t* coversListener);

    static hpatch_inline hpatch_BOOL patch_single_stream_mem(sspatch_listener_t* listener, unsigned char* out_newData, unsigned char* out_newData_end, const unsigned char* oldData, const unsigned char* oldData_end, const unsigned char* diff, const unsigned char* diff_end, sspatch_coversListener_t* coversListener)
    {
        hpatch_TStreamOutput out_newStream;
        hpatch_TStreamInput oldStream;
        hpatch_TStreamInput diffStream;
    ...
    // singleCompressedDiff create by create_single_compressed_diff() or create_single_compressed_diff_stream()
    // singleCompressedDiff: sequential read
    // diffInfo_pos: default 0, begin pos in singleCompressedDiff
    hpatch_BOOL getSingleCompressedDiffInfo(hpatch_singleCompressedDiffInfo* out_diffInfo, const hpatch_TStreamInput* singleCompressedDiff, hpatch_StreamPos_t diffInfo_pos);

    static hpatch_inline hpatch_BOOL getSingleCompressedDiffInfo_mem(hpatch_singleCompressedDiffInfo* out_diffInfo, const unsigned char* singleCompressedDiff, const unsigned char* singleCompressedDiff_end)
    {
        hpatch_TStreamInput diffStream;
    ...
    // coversListener: default NULL, call by on got covers
    hpatch_BOOL patch_single_compressed_diff(const hpatch_TStreamOutput* out_newData, const hpatch_TStreamInput* oldData, const hpatch_TStreamInput* singleCompressedDiff, hpatch_StreamPos_t diffData_pos, hpatch_StreamPos_t uncompressedSize, hpatch_StreamPos_t compressedSize, hpatch_TDecompress* decompressPlugin, hpatch_StreamPos_t coverCount, hpatch_size_t stepMemSize, unsigned char* temp_cache, unsigned char* temp_cache_end, sspatch_coversListener_t* coversListener);

    hpatch_BOOL compressed_stream_as_uncompressed(hpatch_TUncompresser_t* uncompressedStream, hpatch_StreamPos_t uncompressedSize, hpatch_TDecompress* decompressPlugin, const hpatch_TStreamInput* compressedStream, hpatch_StreamPos_t compressed_pos, hpatch_StreamPos_t compressed_end);

    void close_compressed_stream_as_uncompressed(hpatch_TUncompresser_t* uncompressedStream);

    // out_newData: sequential write
    // oldData: random read
    // uncompressedDiffData: sequential read
    // diffData_pos: diffData begin pos in uncompressedDiffData
    // diffData_posEnd: diffData end pos in uncompressedDiffData
    hpatch_BOOL patch_single_stream_diff(const hpatch_TStreamOutput* out_newData, const hpatch_TStreamInput* oldData, const hpatch_TStreamInput* uncompressedDiffData, hpatch_StreamPos_t diffData_pos, hpatch_StreamPos_t diffData_posEnd, hpatch_StreamPos_t coverCount, hpatch_size_t stepMemSize, unsigned char* temp_cache, unsigned char* temp_cache_end, sspatch_coversListener_t* coversListener);

    // byte rle type , ctrl code: high 2bit + packedLen(6bit+...)
    typedef enum TByteRleType
    {
        // 00 rle 0, data code:0 byte
        kByteRleType_rle0 = 0,

        // 01 rle 255, data code:0 byte
        kByteRleType_rle255 = 1,

        // 10 rle x(1--254), data code:1 byte (save x)
        kByteRleType_rle = 2,

        // 11 n byte data, data code:n byte(save no rle data)
    ...
    hpatch_BOOL read_diffz_head(hpatch_compressedDiffInfo* out_diffInfo, _THDiffzHead* out_head, const hpatch_TStreamInput* compressedDiff);
    ...
    static hpatch_inline void _TStreamCacheClip_init(TStreamCacheClip* sclip, const hpatch_TStreamInput* srcStream, hpatch_StreamPos_t streamPos, hpatch_StreamPos_t streamPos_end, unsigned char* aCache, hpatch_size_t cacheSize)
    {
...
#define _TStreamCacheClip_leaveSize(sclip) ((hpatch_StreamPos_t)((sclip)->streamPos_end - (sclip)->streamPos) + (hpatch_StreamPos_t)_TStreamCacheClip_cachedSize(sclip))
#define _TStreamCacheClip_isFinish(sclip) (0 == _TStreamCacheClip_leaveSize(sclip))
#define _TStreamCacheClip_readPosOfSrcStream(sclip) ((sclip)->streamPos - _TStreamCacheClip_cachedSize(sclip))

    hpatch_BOOL _TStreamCacheClip_updateCache(TStreamCacheClip* sclip);

    // error return 0
    static hpatch_inline unsigned char* _TStreamCacheClip_accessData(TStreamCacheClip* sclip, hpatch_size_t readSize)
    {
        // assert(readSize<=sclip->cacheEnd);
        if (readSize > _TStreamCacheClip_cachedSize(sclip))
        {
            if (!_TStreamCacheClip_updateCache(sclip))
            {
                return 0;
            }
...
#define _TStreamCacheClip_skipData_noCheck(sclip,skipSize) ((sclip)->cacheBegin+=skipSize)
    ...
    // error return 0
    static hpatch_inline unsigned char* _TStreamCacheClip_readData(TStreamCacheClip* sclip, hpatch_size_t readSize)
    {
    ...
    hpatch_BOOL _TStreamCacheClip_readDataTo(TStreamCacheClip* sclip, unsigned char* out_buf, unsigned char* bufEnd);
    ...
    hpatch_BOOL _TStreamCacheClip_readType_end(TStreamCacheClip* sclip, unsigned char endTag, char out_type[hpatch_kMaxPluginTypeLength + 1]);
    ...
    static hpatch_inline void _TOutStreamCache_init(_TOutStreamCache* self, const hpatch_TStreamOutput* dstStream, unsigned char* aCache, hpatch_size_t aCacheSize)
    {
        self->writeToPos = 0;
        self->cacheCur = 0;
    ...
    static hpatch_inline void _TOutStreamCache_resetCache(_TOutStreamCache* self, unsigned char* aCache, hpatch_size_t aCacheSize)
    {
    ...
    hpatch_BOOL _TOutStreamCache_copyFromStream(_TOutStreamCache* self, const hpatch_TStreamInput* src, hpatch_StreamPos_t srcPos, hpatch_StreamPos_t copyLength);
    ...
    hpatch_BOOL getStreamClip(TStreamCacheClip* out_clip, _TDecompressInputStream* out_stream, hpatch_StreamPos_t dataSize, hpatch_StreamPos_t compressedSize, const hpatch_TStreamInput* stream, hpatch_StreamPos_t* pCurStreamPos, hpatch_TDecompress* decompressPlugin, unsigned char* aCache, hpatch_size_t cacheSize);
...
#define _TDiffToSingleStream_kBufSize hpatch_kStreamCacheSize
    ...
    static hpatch_inline void TDiffToSingleStream_setInSingleStream(TDiffToSingleStream* self, hpatch_StreamPos_t singleStreamPos)
    {
        self->isInSingleStream = hpatch_TRUE;
    }

    static hpatch_inline void TDiffToSingleStream_resetStream(TDiffToSingleStream* self, const hpatch_TStreamInput* diffStream)
    {
...
#define _SAFE_CHECK_DO(code) \
    ...
    // ......
    // 写入整数并前进指针.
    hpatch_BOOL hpatch_packUIntWithTag(TByte** out_code, TByte* out_code_end, hpatch_StreamPos_t uValue, hpatch_uint highTag, const hpatch_uint kTagBit)
    {
        ...
        *pcode = (TByte)((TByte)uValue | (highTag << (8 - kTagBit)) | (((codeBuf != codeEnd) ? 1 : 0) << (7 - kTagBit)));
        ++pcode;
    ...
    // 读出整数并前进指针.
    hpatch_BOOL hpatch_unpackUIntWithTag(const TByte** src_code, const TByte* src_code_end, hpatch_StreamPos_t* result, const hpatch_uint kTagBit)
    {
    ...
    static hpatch_BOOL _read_mem_stream(const hpatch_TStreamInput* stream, hpatch_StreamPos_t readFromPos, unsigned char* out_data, unsigned char* out_data_end)
    {
        const unsigned char* src = (const unsigned char*)stream->streamImport;
        hpatch_size_t readLen = out_data_end - out_data;
        if (readFromPos > stream->streamSize)
        {
            return _hpatch_FALSE;
        }

        if (readLen > (hpatch_StreamPos_t)(stream->streamSize - readFromPos))
        {
            return _hpatch_FALSE;
        }
    ...
    static hpatch_BOOL _write_mem_stream(const hpatch_TStreamOutput* stream, hpatch_StreamPos_t writeToPos, const unsigned char* data, const unsigned char* data_end)
    {
        unsigned char* out_dst = (unsigned char*)stream->streamImport;
        hpatch_size_t writeLen = data_end - data;

        if (writeToPos > stream->streamSize)
        {
            return _hpatch_FALSE;
        }

        if (writeLen > (hpatch_StreamPos_t)(stream->streamSize - writeToPos))
        {
            return _hpatch_FALSE;
        }
    ...
    typedef hpatch_BOOL(*_read_mem_stream_t)(const hpatch_TStreamOutput* stream, hpatch_StreamPos_t readFromPos, unsigned char* out_data, unsigned char* out_data_end);
    ...
    hpatch_BOOL hpatch_deccompress_mem(hpatch_TDecompress* decompressPlugin, const unsigned char* code, const unsigned char* code_end, unsigned char* out_data, unsigned char* out_data_end)
    {
        ...
        dec = decompressPlugin->open(decompressPlugin, (out_data_end - out_data), &codeStream, 0, codeStream.streamSize);
        if (dec == 0)
        {
            return _hpatch_FALSE;
        }
    ...
    static hpatch_inline hpatch_BOOL _unpackUIntWithTag(const TByte** src_code, const TByte* src_code_end, hpatch_size_t* result, const hpatch_uint kTagBit)
    {
        if constexpr (sizeof(hpatch_size_t) == sizeof(hpatch_StreamPos_t))
        {
...
#define unpackUIntWithTagTo(puint, src_code, src_code_end, kTagBit) _SAFE_CHECK_DO(_unpackUIntWithTag(src_code, src_code_end, puint, kTagBit))
#define unpackUIntTo(puint, src_code, src_code_end) unpackUIntWithTagTo(puint, src_code, src_code_end, 0)

    hpatch_BOOL patch(TByte* out_newData, TByte* out_newData_end, const TByte* oldData, const TByte* oldData_end, const TByte* serializedDiff, const TByte* serializedDiff_end)
    {
        const TByte
            * code_lengths, * code_lengths_end,
        ...
        unpackUIntTo(&coverCount, &serializedDiff, serializedDiff_end);
        {
            ...
            if (lengthSize > (hpatch_size_t)(serializedDiff_end - serializedDiff))
            {
                return _hpatch_FALSE;
            }
            ...
            if (inc_newPosSize > (hpatch_size_t)(serializedDiff_end - serializedDiff))
            {
                return _hpatch_FALSE;
            }
            ...
            if (inc_oldPosSize > (hpatch_size_t)(serializedDiff_end - serializedDiff))
            {
                return _hpatch_FALSE;
            }
            ...
            if (newDataDiffSize > (hpatch_size_t)(serializedDiff_end - serializedDiff))
            {
                return _hpatch_FALSE;
            }
...

        {
            ...
            for (i = 0; i < coverCount; ++i)
            {
                ...
                if (copyLength > 0)
                {
                    if (copyLength > (hpatch_size_t)(newDataSize - newPosBack))
                    {
                        return _hpatch_FALSE;
                    }

                    if (copyLength > (hpatch_size_t)(code_newDataDiff_end - code_newDataDiff))
                    {
                        return _hpatch_FALSE;
                    }
                ...
                if ((addLength > (hpatch_size_t)(newDataSize - newPosBack)))
                {
                    return _hpatch_FALSE;
                }

                if ((oldPos > (hpatch_size_t)(oldData_end - oldData)) || (addLength > (hpatch_size_t)(oldData_end - oldData - oldPos)))
                {
                    return _hpatch_FALSE;
                }
    ...
    static hpatch_inline void addData(TByte* dst, const TByte* src, hpatch_size_t length)
    {
        while (length--)
        {
            *dst++ += *src++;
    ...
    static hpatch_BOOL _bytesRle_load(TByte* out_data, TByte* out_dataEnd, const TByte* rle_code, const TByte* rle_code_end)
    {
        ...
        if (ctrlSize > (hpatch_size_t)(rle_code_end - rle_code))
        {
            return _hpatch_FALSE;
        }
        ...
        while (ctrlBuf_end - ctrlBuf > 0)
        {
            ...
            if (length >= (hpatch_size_t)(out_dataEnd - out_data))
            {
                return _hpatch_FALSE;
            }
            ...
            case kByteRleType_rle:
                if (1 > (hpatch_size_t)(rle_code_end - rle_code))
                {
                    return _hpatch_FALSE;
                }
            ...
            case kByteRleType_unrle:
                if (length > (hpatch_size_t)(rle_code_end - rle_code))
                {
                    return _hpatch_FALSE;
                }
    ...
    static hpatch_BOOL _TStreamInputClip_read(const hpatch_TStreamInput* stream, hpatch_StreamPos_t readFromPos, unsigned char* out_data, unsigned char* out_data_end)
    {
    ...
    static hpatch_BOOL _TStreamOutputClip_write(const hpatch_TStreamOutput* stream, hpatch_StreamPos_t writePos, const unsigned char* data, const unsigned char* data_end)
    {
        TStreamOutputClip* self = (TStreamOutputClip*)stream->streamImport;
        if (writePos + (data_end - data) > self->base.streamSize)
        {
            return _hpatch_FALSE;
        }
    ...
    static_assert(hpatch_kStreamCacheSize >= (hpatch_kMaxPluginTypeLength + 1), "");

    hpatch_BOOL _TStreamCacheClip_readType_end(TStreamCacheClip* sclip, TByte endTag, char out_type[hpatch_kMaxPluginTypeLength + 1])
    {
    ...
    hpatch_BOOL _TStreamCacheClip_updateCache(TStreamCacheClip* sclip)
    {
        ...
        {
            memmove(buf0 + (hpatch_size_t)(sclip->cacheBegin - readSize), buf0 + sclip->cacheBegin, _TStreamCacheClip_cachedSize(sclip));
        }

        // read error
        if (!sclip->srcStream->read(sclip->srcStream, sclip->streamPos, buf0 + (sclip->cacheEnd - readSize), buf0 + sclip->cacheEnd))
        {
            return _hpatch_FALSE;
        }
    ...
    static_assert(hpatch_kStreamCacheSize >= hpatch_kMaxPackedUIntBytes, "");
    ...
    hpatch_BOOL _TStreamCacheClip_readDataTo(TStreamCacheClip* sclip, TByte* out_buf, TByte* bufEnd)
    {
        ...
        if (outLen)
        {
            ...
            if (outLen < (sclip->cacheEnd >> 1))
            {
                ...
                if (outLen > _TStreamCacheClip_cachedSize(sclip))
                {
                    return _hpatch_FALSE;
                }
            ...
            {
                if (!sclip->srcStream->read(sclip->srcStream, sclip->streamPos, out_buf, bufEnd))
                {
                    return _hpatch_FALSE;
                }
    ...
    static hpatch_BOOL _decompress_read(const hpatch_TStreamInput* stream, const hpatch_StreamPos_t  readFromPos, TByte* out_data, TByte* out_data_end)
    {
    ...
    hpatch_BOOL getStreamClip(TStreamCacheClip* out_clip, _TDecompressInputStream* out_stream, hpatch_StreamPos_t dataSize, hpatch_StreamPos_t compressedSize, const hpatch_TStreamInput* stream, hpatch_StreamPos_t* pCurStreamPos, hpatch_TDecompress* decompressPlugin, TByte* aCache, hpatch_size_t cacheSize)
    {
        ...
        else
        {
            ...
            if (out_clip)
            {
                ...
                {
                    if (decompressPlugin->reset_code == 0)
                    {
                        return _hpatch_FALSE;
                    }

                    if (!decompressPlugin->reset_code(out_stream->decompressHandle, dataSize, stream, curStreamPos, curStreamPos + compressedSize))
                    {
                        return _hpatch_FALSE;
                    }
                }
    ...
    hpatch_BOOL _TOutStreamCache_copyFromSelf(_TOutStreamCache* self, hpatch_StreamPos_t aheadLength, hpatch_StreamPos_t copyLength)
    {
        ...
        if (srcPos + copyLength <= self->writeToPos)
        {
            //    [        copyLength          ]
        __copy_in_stream:
            return _TOutStreamCache_copyFromStream(self, src, srcPos, copyLength);
        }
        else if (srcPos >= self->writeToPos)
        {
            //                                      [        copyLength        ]
        __copy_in_mem:
            while (copyLength > 0)
    ...
    static hpatch_inline void _TBytesRle_load_stream_init(_TBytesRle_load_stream* loader)
    {
    ...
    static hpatch_inline void memSet_add(TByte* dst, const TByte src, hpatch_size_t length)
    {
        while (length--)
        {
            (*dst++) += src;
    ...
    static hpatch_BOOL _TBytesRle_load_stream_mem_add(_TBytesRle_load_stream* loader, hpatch_size_t* _decodeSize, TByte** _out_data)
    {
    ...
    static hpatch_inline hpatch_BOOL _TBytesRle_load_stream_isFinish(const _TBytesRle_load_stream* loader)
    {
...
#define _clip_unpackUIntWithTagTo(puint, sclip, kTagBit) \
    { \
        if (!_TStreamCacheClip_unpackUIntWithTag(sclip, puint, kTagBit)) \
        { \
            return _hpatch_FALSE; \
        } \
    }
#define _clip_unpackUIntTo(puint, sclip) _clip_unpackUIntWithTagTo(puint, sclip, 0)
    ...
    static hpatch_BOOL _TBytesRle_load_stream_decode_add(_TBytesRle_load_stream* loader, TByte* out_data, hpatch_size_t decodeSize)
    {
...
#define _TBytesRle_load_stream_decode_skip(loader, decodeSize) _TBytesRle_load_stream_decode_add(loader, 0, decodeSize)

    static hpatch_BOOL _patch_add_old_with_rle(_TOutStreamCache* outCache, _TBytesRle_load_stream* rle_loader, const hpatch_TStreamInput* old, hpatch_StreamPos_t oldPos, hpatch_StreamPos_t addLength, TByte* aCache, hpatch_size_t aCacheSize)
    {
        while (addLength > 0)
        {
            ...
            if (!old->read(old, oldPos, aCache, aCache + decodeStep))
            {
                return _hpatch_FALSE;
            }

            if (!_TBytesRle_load_stream_decode_add(rle_loader, aCache, decodeStep))
            {
                return _hpatch_FALSE;
            }

            if (!_TOutStreamCache_write(outCache, aCache, decodeStep))
            {
                return _hpatch_FALSE;
            }
    ...
    static void _covers_init(_TCovers* covers, hpatch_StreamPos_t coverCount, TStreamCacheClip* code_inc_oldPosClip, TStreamCacheClip* code_inc_newPosClip, TStreamCacheClip* code_lengthsClip, hpatch_BOOL isOldPosBackNeedAddLength)
    {
    ...
    static hpatch_BOOL _rle_decode_skip(struct _TBytesRle_load_stream* rle_loader, hpatch_StreamPos_t copyLength)
    {
        while (copyLength > 0)
        {
            ...
            if (!_TBytesRle_load_stream_decode_skip(rle_loader, len))
            {
                return _hpatch_FALSE;
            }
    ...
    static hpatch_BOOL patchByClip(_TOutStreamCache* outCache, const hpatch_TStreamInput* oldData, hpatch_TCovers* covers, TStreamCacheClip* code_newDataDiffClip, struct _TBytesRle_load_stream* rle_loader, TByte* temp_cache, hpatch_size_t cache_size)
    {
        ...
        {
            hpatch_TCover cover;
            if (!covers->read_cover(covers, &cover))
            {
                return _hpatch_FALSE;
            }
            ...
            if (cover.length > (hpatch_StreamPos_t)(newDataSize - cover.newPos))
            {
                return _hpatch_FALSE;
            }
            ...
            if (cover.length > (hpatch_StreamPos_t)(oldDataSize - cover.oldPos))
            {
                return _hpatch_FALSE;
            }
            ...
            if (newPosBack < cover.newPos)
            {
                ...
                if (!_TOutStreamCache_copyFromClip(outCache, code_newDataDiffClip, copyLength))
                {
                    return _hpatch_FALSE;
                }

                if (!_rle_decode_skip(rle_loader, copyLength))
                {
                    return _hpatch_FALSE;
                }
            }

            if (!_patch_add_old_with_rle(outCache, rle_loader, oldData, cover.oldPos, cover.length, temp_cache, cache_size))
            {
                return _hpatch_FALSE;
            }
...
#define _cache_alloc(dst, dst_type, memSize, temp_cache, temp_cache_end) \
    { \
        if ((hpatch_size_t)(temp_cache_end - temp_cache) < sizeof(hpatch_StreamPos_t) + (memSize)) \
        { \
            return hpatch_FALSE; \
        } \
     \
    ...
    static hpatch_BOOL read_diff_head(_THDiffHead* out_diffHead, const hpatch_TStreamInput* serializedDiff)
    {
        ...
        if (out_diffHead->lengthSize > (hpatch_StreamPos_t)(diffPos_end - diffPos0))
        {
            return _hpatch_FALSE;
        }

        diffPos0 += out_diffHead->lengthSize;
        if (out_diffHead->inc_newPosSize > (hpatch_StreamPos_t)(diffPos_end - diffPos0))
        {
            return _hpatch_FALSE;
        }

        diffPos0 += out_diffHead->inc_newPosSize;
        if (out_diffHead->inc_oldPosSize > (hpatch_StreamPos_t)(diffPos_end - diffPos0))
        {
            return _hpatch_FALSE;
        }
        ...
        if (out_diffHead->newDataDiffSize > (hpatch_StreamPos_t)(diffPos_end - diffPos0))
        {
            return _hpatch_FALSE;
        }
    ...
    static hpatch_BOOL _packedCovers_open(_TPackedCovers** out_self, _THDiffHead* out_diffHead, const hpatch_TStreamInput* serializedDiff, TByte* temp_cache, TByte* temp_cache_end)
    {
        ...
        {
            hpatch_StreamPos_t diffPos0;
            if (!read_diff_head(out_diffHead, serializedDiff))
            {
                return _hpatch_FALSE;
            }

            diffPos0 = out_diffHead->headEndPos;
            _TStreamCacheClip_init(&self->code_lengthsClip, serializedDiff, diffPos0, diffPos0 + out_diffHead->lengthSize, temp_cache, cacheSize);
            temp_cache += cacheSize;
            diffPos0 += out_diffHead->lengthSize;
            _TStreamCacheClip_init(&self->code_inc_newPosClip, serializedDiff, diffPos0, diffPos0 + out_diffHead->inc_newPosSize, temp_cache, cacheSize);
            temp_cache += cacheSize;
            diffPos0 += out_diffHead->inc_newPosSize;
            _TStreamCacheClip_init(&self->code_inc_oldPosClip, serializedDiff, diffPos0, diffPos0 + out_diffHead->inc_oldPosSize, temp_cache, cacheSize);
        }

        _covers_init(&self->base, out_diffHead->coverCount, &self->code_inc_oldPosClip, &self->code_inc_newPosClip, &self->code_lengthsClip, hpatch_FALSE);
        *out_self = self;
        return hpatch_TRUE;
    }

    static hpatch_BOOL _patch_stream_with_cache(const hpatch_TStreamOutput* out_newData, const hpatch_TStreamInput* oldData, const hpatch_TStreamInput* serializedDiff, hpatch_TCovers* cached_covers, TByte* temp_cache, TByte* temp_cache_end)
    {
        ...
        if (cached_covers == 0)
        {
            ...
            if (!_packedCovers_open(&packedCovers, &diffHead, serializedDiff, temp_cache + cacheSize * (_kCachePatCount - 3), temp_cache_end))
            {
                return _hpatch_FALSE;
            }

            // not need close before return
            pcovers = &packedCovers->base.ICovers;
        }
        else
        {
            pcovers = cached_covers;
            if (!read_diff_head(&diffHead, serializedDiff))
            {
                return _hpatch_FALSE;
            }
        }
        ...
        _TStreamCacheClip_init(&code_newDataDiffClip, serializedDiff, diffPos0, diffPos0 + diffHead.newDataDiffSize, temp_cache, cacheSize);
        temp_cache += cacheSize;
...

        {
            ...
            // rename, share address
            TStreamCacheClip* rleHeadClip = &rle_loader.ctrlClip;
            if (cacheSize < hpatch_kMaxPackedUIntBytes)
            {
                return _hpatch_FALSE;
            }

            _TStreamCacheClip_init(rleHeadClip, serializedDiff, diffPos0, diffPos_end, temp_cache, hpatch_kMaxPackedUIntBytes);
            _clip_unpackUIntTo(&rleCtrlSize, rleHeadClip);
            rlePos0 = (hpatch_StreamPos_t)(_TStreamCacheClip_readPosOfSrcStream(rleHeadClip));
            if (rleCtrlSize > (hpatch_StreamPos_t)(diffPos_end - rlePos0))
            {
                return _hpatch_FALSE;
            }

            _TBytesRle_load_stream_init(&rle_loader);
            _TStreamCacheClip_init(&rle_loader.ctrlClip, serializedDiff, rlePos0, rlePos0 + rleCtrlSize, temp_cache, cacheSize);
            temp_cache += cacheSize;
            _TStreamCacheClip_init(&rle_loader.rleCodeClip, serializedDiff, rlePos0 + rleCtrlSize, diffPos_end, temp_cache, cacheSize);
            temp_cache += cacheSize;
        }

        {
            ...
            temp_cache += cacheSize;
            return patchByClip(&outCache, oldData, pcovers, &code_newDataDiffClip, &rle_loader, temp_cache, cacheSize);
        }
    }
    ...
    hpatch_BOOL read_diffz_head(hpatch_compressedDiffInfo* out_diffInfo, _THDiffzHead* out_head, const hpatch_TStreamInput* compressedDiff)
    {
        ...
        _TStreamCacheClip_init(&_diffHeadClip, compressedDiff, 0, compressedDiff->streamSize, temp_cache, hpatch_kStreamCacheSize);
        {
            ...
            if (!_TStreamCacheClip_readType_end(diffHeadClip, '&', tempType))
            {
                return _hpatch_FALSE;
            }
        ...
        {
            // read compressType
            if (!_TStreamCacheClip_readType_end(diffHeadClip, '\0', out_diffInfo->compressType))
            {
                return _hpatch_FALSE;
            }
        ...
        out_diffInfo->compressedCount =
            ((out_head->compress_cover_buf_size) ? 1 : 0) +
...
#define _kCacheDecCount 6
    ...
    static hpatch_BOOL _patch_decompress_cache(const hpatch_TStreamOutput* out_newData, hpatch_TStreamInput* once_in_newData, const hpatch_TStreamInput* oldData, const hpatch_TStreamInput* compressedDiff, hpatch_TDecompress* decompressPlugin, hpatch_TCovers* cached_covers, TByte* temp_cache, TByte* temp_cache_end)
    {
        ...
        {
            // head
            if (!read_diffz_head(&diffInfo, &head, compressedDiff))
            {
                return _hpatch_FALSE;
            }
        ...
        {
            if (!getStreamClip(&coverClip, &decompressers[0], head.cover_buf_size, head.compress_cover_buf_size, compressedDiff, &diffPos0, decompressPlugin, temp_cache + cacheSize * (_kCacheDecCount - 1), cacheSize))
            {
                _clear_return(_hpatch_FALSE);
            }
        }

        if (!getStreamClip(&rle_loader.ctrlClip, &decompressers[1], head.rle_ctrlBuf_size, head.compress_rle_ctrlBuf_size, compressedDiff, &diffPos0, decompressPlugin, temp_cache, cacheSize))
        {
            _clear_return(_hpatch_FALSE);
        }
        ...
        if (!getStreamClip(&rle_loader.rleCodeClip, &decompressers[2], head.rle_codeBuf_size, head.compress_rle_codeBuf_size, compressedDiff, &diffPos0, decompressPlugin, temp_cache, cacheSize))
        {
            _clear_return(_hpatch_FALSE);
        }
        ...
        if (!getStreamClip(&code_newDataDiffClip, &decompressers[3], head.newDataDiff_size, head.compress_newDataDiff_size, compressedDiff, &diffPos0, decompressPlugin, temp_cache, cacheSize))
        {
            _clear_return(_hpatch_FALSE);
        }
    ...
    static hpatch_inline hpatch_BOOL _cache_load_all(const hpatch_TStreamInput* data, TByte* cache, TByte* cache_end)
    {
    ...
    static hpatch_BOOL _compressedCovers_open(_TCompressedCovers** out_self, hpatch_compressedDiffInfo* out_diffInfo, const hpatch_TStreamInput* compressedDiff, hpatch_TDecompress* decompressPlugin, TByte* temp_cache, TByte* temp_cache_end)
    {
        _THDiffzHead head;
        hpatch_StreamPos_t diffPos0 = 0;
        _TCompressedCovers* self = 0;
        _cache_alloc(self, _TCompressedCovers, sizeof(_TCompressedCovers), temp_cache, temp_cache_end);
        if (!read_diffz_head(out_diffInfo, &head, compressedDiff))
        {
            return _hpatch_FALSE;
        }
        ...
        if (head.compress_cover_buf_size > 0)
        {
            ...
            if (!decompressPlugin->is_can_open(out_diffInfo->compressType))
            {
                return _hpatch_FALSE;
            }
        }

        _covers_init(&self->base, head.coverCount, &self->coverClip, &self->coverClip, &self->coverClip, hpatch_TRUE);
        self->base.ICovers.close = _compressedCovers_close;
        memset(&self->decompresser, 0, sizeof(self->decompresser));
        if (!getStreamClip(&self->coverClip, &self->decompresser, head.cover_buf_size, head.compress_cover_buf_size, compressedDiff, &diffPos0, decompressPlugin, temp_cache, temp_cache_end - temp_cache))
        {
            return _hpatch_FALSE;
        }
...
#define _arrayCovers_set(self, i, item, v) \
{ \
    if ((self)->is32) \
    { \
        ((hpatch_uint32_t*)(self)->pCCovers)[(i) * 4 + (item)] = (hpatch_uint32_t)(v); \
    } \
    else \
    { \
...
#define _arrayCovers_set_cachePos(self, i, v) _arrayCovers_set(self, i, 3, v)
    ...
    static hpatch_inline hpatch_StreamPos_t arrayCovers_memSize(hpatch_StreamPos_t coverCount, hpatch_BOOL is32)
    {
    ...
    static hpatch_BOOL _arrayCovers_load(_TArrayCovers** out_self, hpatch_TCovers* src_covers, hpatch_BOOL isUsedCover32, hpatch_BOOL* out_isReadError, TByte** ptemp_cache, TByte* temp_cache_end)
    {
...
#define __CALL_BACK_C __cdecl
    ...
    static hpatch_size_t _getMaxCachedLen(const _TArrayCovers* src_covers, TByte* temp_cache, TByte* temp_cache_end, TByte* cache_buf_end)
    {
        // 允许缓存的最长单个数据长度;
        const hpatch_size_t kMaxCachedLen = ~((hpatch_size_t)0);
        ...
        // fail return 0
        _cache_alloc(cur_covers.pCCovers, void, memSize, temp_cache, temp_cache_end);
    ...
    static hpatch_size_t _set_cache_pos(_TArrayCovers* covers, hpatch_size_t maxCachedLen, hpatch_StreamPos_t* poldPosBegin, hpatch_StreamPos_t* poldPosEnd)
    {
        const hpatch_size_t coverCount = covers->coverCount;

        // 控制最小缓存数量,否则缓存的意义太小;
        const hpatch_size_t kMinCacheCoverCount = coverCount / 8 + 1;
        ...
        for (i = 0; i < coverCount; ++i)
        {
            ...
            if (clen <= maxCachedLen)
            {
                ...
                if (oldPos < oldPosBegin)
                {
                    oldPosBegin = oldPos;
                }

                if (oldPos + clen > oldPosEnd)
                {
    ...
    static hpatch_BOOL _cache_old_load(const hpatch_TStreamInput* oldData, hpatch_StreamPos_t oldPos, hpatch_StreamPos_t oldPosAllEnd, _TArrayCovers* arrayCovers, hpatch_size_t maxCachedLen, hpatch_size_t sumCacheLen, TByte* old_cache, TByte* old_cache_end, TByte* cache_buf_end)
    {
        // 跳过seekTime*speed长度的空间(SSD可以更小)时间上划得来,否则就顺序访问;
        const hpatch_size_t kMinSpaceLen = (1 << (20 + 2));

        // 磁盘页面对齐访问(只和速度有关，但影响不大);
        const hpatch_size_t kAccessPageSize = 4096;
        hpatch_BOOL result = hpatch_TRUE;
        ...
        while ((oldPos < oldPosAllEnd) && (cur_i < coverCount))
        {
            ...
            {
                readLen = (hpatch_size_t)(oldPosAllEnd - oldPos);
            }

            // error
            if (!oldData->read(oldData, oldPos, cache_buf, cache_buf + readLen))
            {
                result = _hpatch_FALSE; break;
            }
    ...
    static hpatch_BOOL _cache_old_StreamInput_read(const hpatch_TStreamInput* stream, hpatch_StreamPos_t readFromPos, unsigned char* out_data, unsigned char* out_data_end)
    {
        ...
        // error
        if ((readLen > dataLen) || (self->readFromPos != readFromPos))
        {
            return _hpatch_FALSE;
        }
    ...
    static hpatch_BOOL _cache_old(hpatch_TStreamInput** out_cachedOld, const hpatch_TStreamInput* oldData, _TArrayCovers* arrayCovers, hpatch_BOOL* out_isReadError, TByte* temp_cache, TByte** ptemp_cache_end, TByte* cache_buf_end)
    {
        ...
        hpatch_size_t maxCachedLen;
        *out_isReadError = hpatch_FALSE;
        _cache_alloc(*out_cachedOld, hpatch_TStreamInput, sizeof(hpatch_TStreamInput), temp_cache, temp_cache_end);
        _cache_alloc(self, _cache_old_TStreamInput, sizeof(_cache_old_TStreamInput), temp_cache, temp_cache_end);
        ...
        if (!_cache_old_load(oldData, oldPosBegin, oldPosEnd, arrayCovers, maxCachedLen, sumCacheLen, temp_cache, temp_cache_end, cache_buf_end))
        {
            *out_isReadError = hpatch_TRUE;
            return _hpatch_FALSE;
        }
    ...
    static hpatch_BOOL _patch_cache(hpatch_TCovers** out_covers, const hpatch_TStreamInput** poldData, hpatch_StreamPos_t newDataSize, const hpatch_TStreamInput* diffData, hpatch_BOOL isCompressedDiff, hpatch_TDecompress* decompressPlugin, size_t kCacheCount, TByte** ptemp_cache, TByte** ptemp_cache_end, hpatch_BOOL* out_isReadError)
    {
        const hpatch_TStreamInput* oldData = *poldData;
        const hpatch_size_t kMinCacheSize = hpatch_kStreamCacheSize * kCacheCount;

        // 内存足够时比较好的hpatch_kStreamCacheSize值;
        const hpatch_size_t kBestACacheSize = hpatch_kFileIOBufBetterSize;
        ...
        // 尝试激活CacheOld功能的内存下限;
        const hpatch_size_t kActiveCacheOldMemorySize = (_betterActiveSize > _minActiveSize) ? _minActiveSize : (hpatch_size_t)_betterActiveSize;

        TByte* temp_cache = *ptemp_cache;
        TByte* temp_cache_end = *ptemp_cache_end;
        *out_isReadError = hpatch_FALSE;
        if ((hpatch_size_t)(temp_cache_end - temp_cache) >= oldData->streamSize + kMinCacheSize + sizeof(hpatch_TStreamInput) + sizeof(hpatch_StreamPos_t))
        {
            // load all oldData
            hpatch_TStreamInput* replace_oldData = 0;
            _cache_alloc(replace_oldData, hpatch_TStreamInput, sizeof(hpatch_TStreamInput), temp_cache, temp_cache_end);
            if (!_cache_load_all(oldData, temp_cache_end - oldData->streamSize, temp_cache_end))
            {
        ...
        else if ((hpatch_size_t)(temp_cache_end - temp_cache) >= kActiveCacheOldMemorySize)
        {
            ...
            if (isCompressedDiff)
            {
                ...
                if (!_compressedCovers_open(&compressedCovers, &diffInfo, diffData, decompressPlugin, temp_cache_end - kBestACacheSize - sizeof(_TCompressedCovers), temp_cache_end))
                {
                    *out_isReadError = hpatch_TRUE; return _hpatch_FALSE;
                }
            ...
            else
            {
                ...
                if (!_packedCovers_open(&packedCovers, &diffHead, diffData, temp_cache_end - kBestACacheSize * 3 - sizeof(_TPackedCovers), temp_cache_end))
                {
                    *out_isReadError = hpatch_TRUE; return _hpatch_FALSE;
                }
            ...
            if (!_arrayCovers_load(&arrayCovers, *out_covers, isUsedCover32, out_isReadError, &temp_cache, temp_cache_end - kBestACacheSize))
            {
                if (*out_isReadError)
                {
                    return _hpatch_FALSE;
            ...
            else
            {
                ...
                if (!((*out_covers)->close(*out_covers)))
                {
                    return _hpatch_FALSE;
                }
                ...
                *out_covers = &arrayCovers->ICovers;
                ...
                // free compressedCovers or packedCovers memory
                ...
                if (((hpatch_size_t)(temp_cache_end - temp_cache) <= kBestACacheSize * kCacheCount) || (!_cache_old(&replace_oldData, oldData, arrayCovers, out_isReadError, temp_cache, &old_cache_end, temp_cache_end)))
                {
                    if (*out_isReadError)
                    {
                        return _hpatch_FALSE;
    ...
    hpatch_BOOL patch_stream_with_cache(const struct hpatch_TStreamOutput* out_newData, const struct hpatch_TStreamInput* oldData, const struct hpatch_TStreamInput* serializedDiff, TByte* temp_cache, TByte* temp_cache_end)
    {
        hpatch_BOOL result;
        ...
        _patch_cache(&covers, &oldData, out_newData->streamSize, serializedDiff, hpatch_FALSE, 0, _kCachePatCount, &temp_cache, &temp_cache_end, &isReadError);
        if (isReadError)
        {
            return _hpatch_FALSE;
        }
    ...
    hpatch_BOOL patch_stream(const hpatch_TStreamOutput* out_newData, const hpatch_TStreamInput* oldData, const hpatch_TStreamInput* serializedDiff)
    {
        ...
        return _patch_stream_with_cache(out_newData, oldData, serializedDiff, 0, temp_cache, temp_cache + sizeof(temp_cache) / sizeof(TByte));
    }
    ...
    hpatch_BOOL patch_decompress_with_cache(const hpatch_TStreamOutput* out_newData, const hpatch_TStreamInput* oldData, const hpatch_TStreamInput* compressedDiff, hpatch_TDecompress* decompressPlugin, TByte* temp_cache, TByte* temp_cache_end)
    {
        ...
        // need close before return
        hpatch_TCovers* covers = 0;
        hpatch_BOOL isReadError = hpatch_FALSE;
        _patch_cache(&covers, &oldData, out_newData->streamSize, compressedDiff, hpatch_TRUE, decompressPlugin, _kCacheDecCount, &temp_cache, &temp_cache_end, &isReadError);
        if (isReadError)
        {
            return _hpatch_FALSE;
        }

        result = _patch_decompress_cache(out_newData, 0, oldData, compressedDiff, decompressPlugin, covers, temp_cache, temp_cache_end);
        if ((covers != 0) && (!covers->close(covers)))
        {
            result = _hpatch_FALSE;
        }
    ...
    hpatch_BOOL patch_decompress(const hpatch_TStreamOutput* out_newData, const hpatch_TStreamInput* oldData, const hpatch_TStreamInput* compressedDiff, hpatch_TDecompress* decompressPlugin)
    {
        ...
        return _patch_decompress_cache(out_newData, 0, oldData, compressedDiff, decompressPlugin, 0, temp_cache, temp_cache + sizeof(temp_cache) / sizeof(TByte));
    }
    ...
    hpatch_BOOL hpatch_coverList_open_serializedDiff(hpatch_TCoverList* out_coverList, const hpatch_TStreamInput* serializedDiff)
    {
        ...
        if (!_packedCovers_open(&packedCovers, &diffHead, serializedDiff, temp_cache, temp_cache_end))
        {
            return _hpatch_FALSE;
        }
    ...
    hpatch_BOOL hpatch_coverList_open_compressedDiff(hpatch_TCoverList* out_coverList, const hpatch_TStreamInput* compressedDiff, hpatch_TDecompress* decompressPlugin)
    {
        ...
        temp_cache = out_coverList->_buf;
        temp_cache_end = temp_cache + sizeof(out_coverList->_buf);
        if (!_compressedCovers_open(&compressedCovers, &diffInfo, compressedDiff, decompressPlugin, temp_cache, temp_cache_end))
        {
            return _hpatch_FALSE;
        }
...
#define _kCacheSgCount 3
    ...
    hpatch_BOOL patch_single_compressed_diff(const hpatch_TStreamOutput* out_newData, const hpatch_TStreamInput* oldData, const hpatch_TStreamInput* singleCompressedDiff, hpatch_StreamPos_t diffData_pos, hpatch_StreamPos_t uncompressedSize, hpatch_StreamPos_t compressedSize, hpatch_TDecompress* decompressPlugin, hpatch_StreamPos_t coverCount, hpatch_size_t stepMemSize, unsigned char* temp_cache, unsigned char* temp_cache_end, sspatch_coversListener_t* coversListener)
    {
        ...
        if (decompressPlugin)
        {
            if (!compressed_stream_as_uncompressed(&uncompressedStream, uncompressedSize, decompressPlugin, singleCompressedDiff, diffData_pos, diffData_posEnd))
            {
                return _hpatch_FALSE;
            }
    ...
    hpatch_BOOL getSingleCompressedDiffInfo(hpatch_singleCompressedDiffInfo* out_diffInfo, const hpatch_TStreamInput* singleCompressedDiff, hpatch_StreamPos_t diffInfo_pos)
    {
        ...
        TStreamCacheClip* diffHeadClip = &_diffHeadClip;
        TByte temp_cache[hpatch_kStreamCacheSize];
        _TStreamCacheClip_init(&_diffHeadClip, singleCompressedDiff, diffInfo_pos, singleCompressedDiff->streamSize, temp_cache, hpatch_kStreamCacheSize);
    ...
    static hpatch_BOOL _TUncompresser_read(const struct hpatch_TStreamInput* stream, hpatch_StreamPos_t readFromPos, unsigned char* out_data, unsigned char* out_data_end)
    {
    ...
    static hpatch_BOOL _rle0_decoder_add(rle0_decoder_t* self, TByte* out_data, hpatch_size_t decodeSize)
    {
        ...
        _decode_0_process:
            ...
            if (!hpatch_unpackUInt(&self->code, self->code_end, &len0))
            {
                return _hpatch_FALSE;
            }

            if (len0 != (hpatch_size_t)len0)
            {
                return _hpatch_FALSE;
            }
        ...
        _decode_v_process:
            self->isNeedDecode0 = hpatch_TRUE;
            if (!hpatch_unpackUInt(&self->code, self->code_end, &lenv))
            {
                return _hpatch_FALSE;
            }

            if (lenv > (size_t)(self->code_end - self->code))
            {
                return _hpatch_FALSE;
            }
    ...
    static hpatch_BOOL _patch_add_old_with_rle0(_TOutStreamCache* outCache, rle0_decoder_t* rle0_decoder, const hpatch_TStreamInput* old, hpatch_StreamPos_t oldPos, hpatch_StreamPos_t addLength, TByte* aCache, hpatch_size_t aCacheSize)
    {
        while (addLength > 0)
        {
            hpatch_size_t decodeStep = aCacheSize;
            ...
            if (!_rle0_decoder_add(rle0_decoder, aCache, decodeStep))
            {
                return _hpatch_FALSE;
            }
    ...
    hpatch_BOOL sspatch_covers_nextCover(sspatch_covers_t* self)
    {
        ...
        if (!hpatch_unpackUIntWithTag(&self->covers_cache, self->covers_cacheEnd, &self->cover.oldPos, 1))
        {
            return _hpatch_FALSE;
        }
        ...
        if (!hpatch_unpackUInt(&self->covers_cache, self->covers_cacheEnd, &self->cover.newPos))
        {
            return _hpatch_FALSE;
        }

        self->cover.newPos += self->lastNewEnd;
        if (!hpatch_unpackUInt(&self->covers_cache, self->covers_cacheEnd, &self->cover.length))
        {
            return _hpatch_FALSE;
        }
    ...
    hpatch_BOOL patch_single_stream_diff(const hpatch_TStreamOutput* out_newData, const hpatch_TStreamInput* oldData, const hpatch_TStreamInput* uncompressedDiffData, hpatch_StreamPos_t diffData_pos, hpatch_StreamPos_t diffData_posEnd, hpatch_StreamPos_t coverCount, hpatch_size_t stepMemSize, unsigned char* temp_cache, unsigned char* temp_cache_end, sspatch_coversListener_t* coversListener)
    {
        ...
        if (coversListener)
        {
            assert(coversListener->onStepCovers);
        }

        {
            //cache
            if ((size_t)(temp_cache_end - temp_cache) < stepMemSize + hpatch_kStreamCacheSize * _kCacheSgCount)
            {
                return _hpatch_FALSE;
            }
        ...
        while (coverCount)
        {
            ...
            rle0_decoder_t       rle0_decoder;
            {
                ...
                unsigned char* bufRle_cache_end;
                {
                    ...
                    if ((bufCover_size > stepMemSize) || (bufRle_size > stepMemSize) || (bufCover_size + bufRle_size > stepMemSize))
                    {
                        return _hpatch_FALSE;
                    }
            ...
            while (sspatch_covers_isHaveNextCover(&covers))
            {
                //cover loop
                ...
                {
                    if ((covers.cover.oldPos > oldData->streamSize) || (covers.cover.length > (hpatch_StreamPos_t)(oldData->streamSize - covers.cover.oldPos)))
                    {
                        return _hpatch_FALSE;
                    }

                    if (!_patch_add_old_with_rle0(&outCache, &rle0_decoder, oldData, covers.cover.oldPos, covers.cover.length, temp_cache, cache_size))
                    {
                        return _hpatch_FALSE;
                    }
                }
    ...
    static hpatch_BOOL _TDiffToSingleStream_read(const struct hpatch_TStreamInput* stream, hpatch_StreamPos_t readFromPos, unsigned char* out_data, unsigned char* out_data_end)
    {
        // [                                                         |readedSize                  ]
    ...
    hpatch_BOOL patch_single_stream(sspatch_listener_t* listener, const hpatch_TStreamOutput* __out_newData, const hpatch_TStreamInput* oldData, const hpatch_TStreamInput* singleCompressedDiff, hpatch_StreamPos_t  diffInfo_pos, sspatch_coversListener_t* coversListener)
    {
        hpatch_BOOL result = hpatch_TRUE;
        ...
        if (result)
        {
            result = patch_single_compressed_diff(out_newData, oldData, singleCompressedDiff, diffInfo.diffDataPos, diffInfo.uncompressedSize, diffInfo.compressedSize, decompressPlugin, diffInfo.coverCount, (size_t)diffInfo.stepMemSize, temp_cache, temp_cacheEnd, coversListener);
        }
...
EXTERN_C __declspec(dllexport) BOOL APIENTRY NewDataSize(HPatch::hpatch_TStreamInput* diff, ULONG64* pSize)
{
    HPatch::hpatch_compressedDiffInfo diffInfo;
    BOOL result = HPatch::getCompressedDiffInfo(&diffInfo, diff);
    *pSize = diffInfo.newDataSize;
    return result;
}
